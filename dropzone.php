...
class YellowDropzone {
    ...
    public function onLoad($yellow) {
        $this->yellow = $yellow;
        $this->yellow->system->setDefault("dropzoneDir", "media/uploads/");
    ...
    public function onParseContentShortcut($page, $name, $text, $type) {
        ...
        if ($name=="dropzone" && ($type=="block" || $type=="inline")) {
            list($acceptedFiles, $maxFilesize) = $this->yellow->toolbox->getTextArgs($text);
            $uniqid = uniqid();
                ...
                foreach ((array)$_FILES['file']['tmp_name'] as $key => $tempFile) {
                    ...
                    if (in_array(".".$fileType, $allowedExts) && ((array)$_FILES['file']['size'])[$key]<=$maxFilesize) {
                        ...
                        if (!$this->yellow->system->get("dropzoneOverwrite")) {
                            while (file_exists($this->yellow->system->get("dropzoneDir").$target["filename"].($fileCount ? ".".$fileCount : "").".".$target["extension"])) {
                                $fileCount += 1;
                            }
                        }
    ...
    public function onParsePageExtra($page, $name) {
        ...
        if ($name == "header") {
            ...
            foreach (["DefaultMessage", "FileTooBig", "InvalidFileType", "ResponseError"] as $string) {
                $output .= "d.dict".$string." = ".json_encode(preg_replace('/@(\w+)/', '{{$1}}', $this->yellow->text->get("dropzone".$string))).";\n";
            }
